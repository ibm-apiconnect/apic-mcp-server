...
function printUsage() {
  console.error(
    'Usage: node ./.bob/skills/openapi-operation-selector/select-operations.js <openapi-file> [--list | --select-indexes "1,3,4"]'
  );
}
...
function buildOperations(document) {
  ...
  for (const [apiPath, pathItem] of Object.entries(pathsObject)) {
    ...
    for (const methodName of HTTP_METHODS) {
      ...
      const label = operationId && operationIdCounts.get(operationId) === 1
        ? operationId
        : `${method} ${apiPath}`;
