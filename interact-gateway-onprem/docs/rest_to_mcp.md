...
```text
Convert /path/to/openapi_file.yaml to MCP for these operations: {"/pets": ["get", "post"]} in a new project pet-mcp
```

**Note**: This operation relies on two skills: the [`apic-openapi-operation-selector`](skills.md#apic-openapi-operation-selector) to help select operations to include, and the [`api-studio-project-manager`](skills.md#api-studio-project-manager) to create the studio project required when publishing the generated MCP assets.
